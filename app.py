...
@app.post("/filter_csv")                       ##added from chatgpt
async def filter_csv(
    csv_path: str = Body(..., description="Path to CSV file"),
    filter_column: str = Body(..., description="Column to filter"),
    filter_value: str = Body(..., description="Value to match")
):
    return B10(csv_path, filter_column, filter_value)
...
async def run_task(task: str):
    try:
        ...
        # Phase A tasks (Already implemented)
        if task_code in ["A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8", "A9", "A10"]:
        ...
        # Phase B tasks
            ...
            B4(**arguments)  # New Git operation support
        elif task_code == "B5":
            B5(**arguments)
        elif task_code == "B6":
            B6(**arguments)
            ...
            B8(**arguments)  # New Audio transcription
        elif task_code == "B9":
            B9(**arguments)
        elif task_code == "B10":
            return B10(**arguments)  # New CSV filtering API, returns JSON
